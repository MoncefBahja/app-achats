...
// If search string
if ($search_string) {
    $stmt = $pdo->prepare("SELECT * FROM admin_accounts WHERE user_name LIKE :search_string");
...
if ($order_by) {
    $stmt = $pdo->prepare("SELECT * FROM admin_accounts ORDER BY $filter_col $order_by");
    $stmt->execute();
}
...
$stmt = $pdo->prepare("SELECT COUNT(*) as total FROM admin_accounts WHERE user_name LIKE :search_string");
$stmt->bindValue(':search_string', '%' . $search_string . '%');
$stmt->execute();
$total_pages = ceil($stmt->fetch(PDO::FETCH_ASSOC)['total'] / $pagelimit);

$stmt = $pdo->prepare("SELECT " . implode(", ", $select) . " FROM admin_accounts WHERE user_name LIKE :search_string ORDER BY $filter_col $order_by LIMIT :offset, :limit");
$stmt->bindValue(':search_string', '%' . $search_string . '%');
$stmt->bindValue(':offset', ($page - 1) * $pagelimit, PDO::PARAM_INT);
$stmt->bindValue(':limit', $pagelimit, PDO::PARAM_INT);
$stmt->execute();
$rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
