...
FROM products
INNER JOIN categories ON products.category_id = categories.id
WHERE categories.gender = 'men' LIMIT " .$resultsPerPage. " OFFSET " .( $page-1)*$resultsPerPage) ;
$results -> execute() ;
...
if($page == $count ) {
  ...
  " href="men.php?page=<?php echo $count ?>"><?php echo $count ?> </a>
    <?php
}
else { ?>
